...
class CondUNet(nn.Module):
    def __init__(self, in_ch=11, d_time=256, n_classes=11):
        super().__init__()
        d_te = d_time * 4
        self.time_embed = TimeEmbedding(d_time)
        self.class_embed = nn.Embedding(n_classes, d_time)
        self.class_mlp = nn.Sequential(
            nn.Linear(d_time, d_te), nn.SiLU(), nn.Linear(d_te, d_te)
        )
    ...
    def forward(self, x, t, cls):
        ...
        t_emb = self.time_embed(t)
        cls_emb = self.class_mlp(self.class_embed(cls))
        cond = t_emb + cls_emb
        ...
        d = self.downsample1(d2)
        d3 = self.down3(d, cond)
        d4 = self.down4(d3, cond)
        d = self.downsample2(d4)
        d5 = self.down5(d, cond)
        d6 = self.down6(d5, cond)
        d = self.downsample3(d6)

        m1 = self.mid1(d, cond)
        m2 = self.mid2(m1, cond)

        u = self.upsample3(m2)
        u = self.up6(torch.cat([u, d6], dim=1), cond)
        u = self.up5(torch.cat([u, d5], dim=1), cond)
        u = self.upsample2(u)
        u = self.up4(torch.cat([u, d4], dim=1), cond)
        u = self.up3(torch.cat([u, d3], dim=1), cond)
        u = self.upsample1(u)
        u = self.up2(torch.cat([u, d2], dim=1), cond)
        u = self.up1(torch.cat([u, d1], dim=1), cond)

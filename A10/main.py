...
# torchrun --nproc_per_node=1 --master_addr=127.0.0.1 --master_port=29500 ./main.py
...
dataset = TokenDataset(np.memmap('tinystories_tokens.bin', dtype=np.uint16, mode='r'),
                       max_seq_len)
...
for epoch in range(last_epoch, n_epoch):
    ...
    for inputs, targets in ds_loader:
        ...
        if step > total_steps:
            break
        if step % 50 == 0:
            print(f'epoch {epoch} step {step}: loss={epoch_loss/epoch_total:.4f}', flush=True)
        if step % 100 == 0:
            if rank == 0:
                dist.barrier()
    ...
    print(f"Epoch {epoch}: loss={epoch_loss:.4f} avgloss={sum(loss_counting)/len(loss_counting):.4f}", flush=True)
    if rank == 0:
        dist.barrier()
        print('saving checkpoint...', flush=True)
        torch.save({
